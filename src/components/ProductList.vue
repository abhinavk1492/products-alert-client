      ...
      <div class="row">
        <div class="col-4 mt-3" v-for="(p,i) in products" :key="i">
          ...
          <div class="row">
            ...
            <div class="col-6">
                <button class="btn btn-info" @click="handleAddsubscription(p)">Subscribe</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions(["setProductsByCategoryAction", "addSubscription"]),
    ...mapMutations(["setCurrentCategory", "setCurrentPage"]),
    ...mapMutations({ addProduct: "cart/addProduct" }),
    handleAddProduct(product) {
      this.addProduct(product);
    },
    handleAddsubscription(product) {
      this.addSubscription(product);
    }
  },
  created() {
